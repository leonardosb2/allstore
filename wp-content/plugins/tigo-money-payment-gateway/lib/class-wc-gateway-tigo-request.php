...
class Tigo_Money_Payment_Gateway_Class_Request {
	 ...
	 * @var string
	 */
	protected $live_url = 'https://prod.api.tigo.com/';
	...
	public function get_request_url( $order, $sandbox = false ) {
		...
		if( $token ) {
			...
			if( $sandbox ){
				...
				$url_payment_authorization_request = $this->live_url.'v2/tigo/mfs/payments/authorizations';
			}
	...
	public function get_bearer_token( $order = false, $sandbox = false ){
		...
		if ( $sandbox ) {
			...
			$url = $this->live_url.'v1/oauth/mfs/payments/tokens';			
		}
