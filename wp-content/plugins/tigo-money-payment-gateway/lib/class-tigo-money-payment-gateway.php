...
class Tigo_Money_Payment_Gateway_Class extends WC_Payment_Gateway {
  ...
  public function tigo_wc_made_refund($order = ''){
    ...
    if ( $this->testmode ) {
      ...
      $url = 'https://prod.api.tigo.com/v2/tigo/mfs/payments/transactions/'. $tigo_request->convert_country_code( $wc_order->billing_country ).'/'. get_post_meta($wc_order->get_order_number(), 'mfsTransactionId', true ) .'/'. ( !empty($tx_id) ? $tx_id : 'null');
    }
    // DEL HTTP
